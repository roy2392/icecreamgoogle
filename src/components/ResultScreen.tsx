...
export function ResultScreen({ topping, hasAllergy, onRestart }: ResultScreenProps) {
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-gradient-to-br from-surface-subtle via-background to-accent/20 flex flex-col items-center justify-start pt-8 p-6 pb-[env(safe-area-inset-bottom,1.5rem)] text-center animate-fade-in-up max-w-md mx-auto" dir="rtl">
      {/* Result Image */}
      <div className="mb-6">
        <div className="relative">
          ...
          <ImageWithFallback 
            ...
            className="relative w-56 h-56 object-cover rounded-full drop-shadow-2xl animate-bounce-gentle transform hover:scale-105 transition-all duration-500"
          />
      ...
      {/* Result Title */}
      <h1 className="text-4xl font-bold mb-3 animate-rainbow-text font-fredoka">
        התוספת שלך: {toppingInfo.name}! 🎊
      </h1>

      {/* Description */}
      <p className="text-xl text-muted-foreground mb-6 max-w-sm leading-relaxed animate-fade-in-up font-medium">
        מבוסס על האישיות הייחודית שלך ✨🌈✨
      </p>

      <p className="text-base text-foreground mb-8 max-w-xs animate-slide-in-fun font-medium bg-gradient-to-r from-accent/20 to-secondary/20 p-4 rounded-2xl border-2 border-accent/30">
        {toppingInfo.description} 💫
      </p>

      {/* Allergy Notice */}
      {hasAllergy && (
        <div className="bg-gradient-to-r from-warning/20 via-warning/10 to-accent/20 border-2 border-warning/40 rounded-2xl p-4 mb-6 animate-wiggle shadow-lg">
          <p className="text-base text-warning-foreground font-semibold">
      ...
      {/* Redemption Section */}
      <div className="bg-gradient-to-br from-card via-accent/10 to-secondary/20 rounded-3xl p-6 shadow-rainbow mb-8 w-full max-w-sm animate-scale-bounce border-2 border-accent/30 backdrop-blur-sm">
        <h3 className="text-xl font-bold text-foreground mb-4 animate-rainbow-text font-fredoka">
