...
export function ChatScreen({ onComplete }: ChatScreenProps) {
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] colorful-sprinkles-pattern flex flex-col w-full relative overflow-hidden">
      ...
      {/* Header */}
      <div className="bg-white/90 backdrop-blur-sm border-b border-gray-200 p-4 md:p-6 shadow-sm relative z-10">
        <div className="flex items-center justify-between">
          ...
          <img 
            src={logoImage} 
            alt="Next AICream" 
            className="h-6 sm:h-8 md:h-10 lg:h-12 w-auto max-w-[140px] sm:max-w-[180px] md:max-w-[220px] lg:max-w-[260px]"
          />
      ...
      {/* Messages */}
      <div className="flex-1 p-4 md:p-6 overflow-y-auto relative z-10 overscroll-behavior-y-contain">
      ...
      {showInput && currentQuestionData && (
        <div className="p-4 md:p-6 pb-[env(safe-area-inset-bottom,1rem)] bg-white/98 backdrop-blur-md border-t border-white/60 shadow-lg relative z-10 sticky bottom-0">
          {/* Quick Reply Chips */}
            ...
            <div className="flex flex-col gap-3 mb-4">
              {currentQuestionData.options.map((option, index) => (
                <button
                  ...
                  className="bg-white/90 hover:bg-white/95 border border-white/60 rounded-full py-3 md:py-4 lg:py-5 px-4 md:px-6 lg:px-8 text-right font-fredoka text-sm md:text-base lg:text-lg text-gray-700 transition-colors duration-200 shadow-sm backdrop-blur-sm"
                  dir="rtl"
                >
                  {option.label}
                </button>
          ...
          <form onSubmit={handleInputSubmit} className="flex gap-3 mb-3">
            <button
              ...
              className="bg-[#FD5109] hover:bg-[#E04300] disabled:bg-gray-300 text-white rounded-full h-12 md:h-14 lg:h-16 w-12 md:w-14 lg:w-16 flex items-center justify-center transition-colors duration-200 font-fredoka text-sm md:text-base lg:text-lg"
            >
              שלח
            </button>
            <div className="flex-1">
              <Input
                ...
                className="text-right bg-white/95 border border-white/70 rounded-full h-12 md:h-14 lg:h-16 px-4 md:px-6 lg:px-8 font-fredoka text-sm md:text-base lg:text-lg placeholder:text-gray-400 focus:border-[#FD5109] focus:ring-1 focus:ring-[#FD5109] backdrop-blur-sm shadow-sm"
                dir="rtl"
              />
            </div>
          </form>
